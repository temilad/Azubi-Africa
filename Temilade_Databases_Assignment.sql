...
SELECT  COUNT(transfer_id) FROM transfers WHERE send_amount_currency = 'CFA' AND
	(kind='transfer' OR kind='payment')

--QUESTION 3
--How many different users have sent a transfer in CFA?

SELECT  COUNT(DISTINCT u_id) FROM transfers WHERE send_amount_currency = 'CFA' AND
	(kind='transfer' OR kind='payment')
...
--QUESTION 5
--How many Wave agents were “net depositors” vs. “netwithdrawers”?
...
;
 ...
 --QUESTION 6
 --find the volume of agent transactions createdin the last week, grouped by city.
...
--QUESTION 7
--separate the atx volume by country
...
--QUESTION 8
--find the total volume of transfers sent in the past week, grouped by country and transfer kind.
...
--QUESTION 9
--Then add columns for transaction count and number of unique senders
...
--QUESTION 10
--which wallets have sent more than 10,000,000 CFA in transfers in the last month and how much did they send?
